...
class TerminalAnchor(Gtk.EventBox if hasattr(Gtk, "EventBox") else Gtk.Box):
    """
    Zero-size hidden anchor widget inserted into the location bar by Nautilus.
    Configured with set_no_show_all(True) to ensure zero top-bar allocation.
    """
    ...
    def __init__(self, uri_or_file, window: Gtk.Window, extension):
        ...
        # Force hidden with zero allocation
        if hasattr(self, "set_no_show_all"):
